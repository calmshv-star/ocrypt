...
-- Watch-only addresses remain reusable after their first assignment: route
-- planning distinguishes concurrent payments by exact reserved amount. Report
-- this as usable capacity instead of incorrectly calling it unavailable.
-- The OUT column name is part of PostgreSQL's function return type, so the
-- 000040 function must be replaced rather than changed in place.
...
AS $$
  WITH merchant AS (
    ...
    SELECT w.chain_id,
      ...
      count(a.id) FILTER(WHERE w.status='active' AND
        (a.status='available' OR (w.custody_mode='watch_only' AND a.status='assigned')))::bigint usable_address_count,
  ...
  SELECT m.settlement_currency,r.currency,r.chain_id,r.asset_id,a.symbol,
    ...
    coalesce(w.address_count,0),coalesce(w.usable_address_count,0),
