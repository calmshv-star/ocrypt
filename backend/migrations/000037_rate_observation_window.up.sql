...
-- A rate is reusable for thirty minutes from the provider observation. The
-- checkout path only wakes the collector after that window has elapsed; it
-- never turns repeated order attempts into upstream polling.
...
BEGIN
  ...
  SELECT t.observed_at INTO latest_observed_at
  ...
  IF FOUND AND latest_observed_at>now_at-interval '30 minutes' THEN
...
-- Migration 000036 preserved the old fifteen-minute tick lifetime for its
-- first transition. Reschedule that one transition to the new thirty-minute
-- observation window without collecting another rate early.
UPDATE rate_runtime_jobs j
SET next_attempt_at=GREATEST(clock_timestamp(),COALESCE((
      SELECT t.observed_at+interval '30 minutes'
      FROM rate_runtime_pair_bindings b
