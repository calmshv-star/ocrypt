...
func loadPublicCheckout(ctx context.Context, tx pgx.Tx, hash [32]byte, tenantID, merchantID, intentID, origin string, now time.Time) (result CheckoutSession, err error) {
	...
	err = tx.QueryRow(ctx, `SELECT i.merchant_order_id,i.status::text,i.expires_at,cs.expires_at,COALESCE(cs.selected_route_id::text,''),cs.audience,COALESCE(cs.allowed_origin,''),cs.allowed_actions,GREATEST(i.version,cs.version) FROM checkout_sessions cs JOIN payment_intents i ON i.id=cs.intent_id AND i.tenant_id=cs.tenant_id WHERE cs.token_hash=$1 AND cs.tenant_id=$2 AND cs.merchant_id=$3 AND cs.intent_id=$4 AND cs.revoked_at IS NULL AND cs.expires_at>clock_timestamp()`, hash[:], tenantID, merchantID, intentID).Scan(&result.OrderID, &status, &intentExpiry, &sessionExpiry, &result.SelectedRouteID, &audience, &allowedOrigin, &actions, &result.Version)
	if errors.Is(err, pgx.ErrNoRows) {
		return result, ErrNotFound
	}
	if err != nil {
