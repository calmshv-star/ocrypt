...
type SweepRequestInput = {
    asset_id: string;
    destination: { chain: string; value: string };
    sources: Array<{ address: { chain: string; value: string }; available: string; nonce_ref: string }>;
    fee_quote: string;
};
...
function ScopeGate({ children }: {
    ...
    return <div className={admin.preview ? "financial-scope is-preview" : "financial-scope"}>{children}</div>;
}
...
export function FinancialSweepsPage() {
    ...
    const [draft, setDraft] = useState<SweepRequestInput | null>(null);
    useEffect(() => { setReason(""); setSourceRows([{ id: crypto.randomUUID() }]); setDraft(null); }, [scope?.tenantId]);
    const create = (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        if (!scope || admin.preview)
            return;
        const data = new FormData(event.currentTarget);
        setDraft({ asset_id: String(data.get("asset")).trim(), destination: { chain: String(data.get("chain")).trim(), value: String(data.get("destination")).trim() }, sources: sourceRows.map(row => ({ address: { chain: String(data.get(`source-chain-${row.id}`)).trim(), value: String(data.get(`source-value-${row.id}`)).trim() }, available: String(data.get(`source-available-${row.id}`)).trim(), nonce_ref: String(data.get(`source-nonce-${row.id}`)).trim() })), fee_quote: String(data.get("fee")).trim() });
    };
    const confirm = () => {
        if (!scope || !draft || admin.preview)
            return;
        const fp = `sweep.create:${JSON.stringify(draft)}`;
        run(fp, async key => { await admin.client!.createFinancialSweep(scope, draft, key); setDraft(null); });
    };
