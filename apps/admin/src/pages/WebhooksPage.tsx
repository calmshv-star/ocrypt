...
export function WebhooksPage() {
  ...
  const columns: DataTableColumn<Delivery>[] = [
    ...
    { key: "action", header: "", render: (delivery) => <Button disabled={delivery.status === "delivered"} size="sm" variant="quiet"><RotateCcw size={13} />{t("common.retry")}</Button> }
  ];

  return (
    <div className="admin-page">
      <PageHeader actions={<Button><Plus size={15} />{t("webhooks.addEndpoint")}</Button>} description={t("page.webhooks.description")} eyebrow={<><Webhook size={13} />{t("webhooks.signedEvents")}</>} title={t("page.webhooks.title")} />
      ...
      <section className="webhook-endpoint-grid">
        {webhookEndpoints.map((endpoint) => (
          <SectionCard action={<StatusBadge status={endpoint.status}>{t(endpoint.status === "healthy" ? "status.healthy" : "status.degraded")}</StatusBadge>} className="webhook-endpoint-card" key={endpoint.id} title={endpoint.merchant}>
            ...
            <div className="webhook-endpoint-card__foot"><span>{relativeTime(endpoint.lastEvent)}</span><Button size="sm" variant="quiet">{t("common.inspect")}</Button></div>
          </SectionCard>
        ))}
      </section>
      ...
      <div className="webhook-security-strip"><span><RefreshCw size={17} /></span><div><strong>{t("webhooks.rotationActive")}</strong><p>{t("webhooks.rotationBody")}</p></div><Button variant="secondary">{t("webhooks.manageKeys")}</Button></div>
    </div>
  );
}
