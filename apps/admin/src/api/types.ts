...
export type FinancialSettingsWallet = { id:string;chain_id:string;chain_name:string;address:string;status:"active"|"disabled"|"quarantined";version:number };
export type FinancialSettingsInventory = { settlement_currency:string;accepted_currencies:string[];routes:FinancialSettingsRoute[];wallets:FinancialSettingsWallet[] };
export type WatchWalletImportKind = "evm_personal_sign"|"solana_sign_message";
export type WatchWalletImportItem = { wallet_id:string;chain_id:string;address:string;version:number };
export type WatchWalletImportChallenge = { kind:WatchWalletImportKind;address:string;wallets:WatchWalletImportItem[];nonce:string;issued_at:string;expires_at:string;message:string;token:string };
export type WatchWalletImportResult = { wallets:FinancialSettingsWallet[] };
