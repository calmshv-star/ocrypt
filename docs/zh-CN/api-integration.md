...
Standalone 安装会为每个内置支付资产预先准入上述六种币种。系统比较 CoinGecko 与 CoinPaprika 的观测；KZT 还使用哈萨克斯坦国家银行发布的每日官方 USD/KZT 汇率。其他三字母代码本身不会生成报价：生产环境仍须为精确的 `asset_id`/币种对提供新鲜且已准入的汇率。缺失、过期、未达到 quorum、来自未来或价差过大的汇率都会 fail closed。
